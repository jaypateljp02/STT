...
app.use(cors());
// Support large payloads up to 100MB
...
// Multer storage setup with 100MB file size limit
  ...
  upload.single('audioFile')(req, res, (err) => {
    if (err) {
      if (err.code === 'LIMIT_FILE_SIZE') {
        return res.status(413).json({ error: 'File size exceeds limit (100MB). For large files, please use the Google Drive Link option.' });
      }
